...
while($row = do_mysqli_fetch("1",$result))
{
    ...
    try {
        ...
        echo "arn=$arn";
        if( $arn!=''){
            do_mysqli_query("1","update notifytokens set arn='$arn', status='Y' where providerid=$providerid and token='$token' ");
        }
    } catch (Exception $e) {
        echo 'Caught exception: ',  $e->getMessage(), "\n";
        $errsql = htmlentities($e->getMessage(), ENT_COMPAT);
        do_mysqli_query("1","update notifytokens set status='E', error = '$errsql' where providerid=$providerid and token='$token' ");
    } 
    echo "<br>";
}
//echo "<br>End";
