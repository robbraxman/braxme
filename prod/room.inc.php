        ...
        $video, $photo, $link, $anonymous, $articleid )
{
        ...
        if($profileflag == 'Y'){
            do_mysqli_query("1","update provider set lastactive = now() where providerid = $posterid ");
        }    
...
function DetermineRoomStage()
{
    if( $_SESSION['roomuser']!=''){
        $result = do_mysqli_query("1","select count(*) as count from statusroom where owner = $_SESSION[pid]  ");
        ...
        if( intval($row['count']) == 1){
            $_SESSION['roomuser'] = '1';
            $result = do_mysqli_query("1","select count(*) as count from statuspost where owner = $_SESSION[pid]  ");
            $row = do_mysqli_fetch("1",$result);
            if(intval($row['count']>0) )
            {
    ...
    if( $_SESSION['roomuser']=='N' ){
        $result = do_mysqli_query("1","select count(*) as count from statusroom where providerid = $_SESSION[pid]  ");
        $row = do_mysqli_fetch("1",$result);
        if( intval($row['count']) > 0){
            $_SESSION['roomuser'] = 'M';
        }
...
function ShareRoom ( $readonly, $caller, $providerid, $roomid, $style, $showroomtip, $callerflag, $owner )
{
    ...
    if( $private === 'Y' && $owned!=='Y' ){
        ...
        $returntext .= "
            <br><br>
            <span class='smalltext groupinvitelinkgroup'  style='display:none;color:$global_textcolor'>
                <b>Room Invite URL to Share</b></span><br>
...
function RoomPosterInfo( $roomid, $providerid, $avatarurl, $adminroom, $private, $anonymous, $roomanonymous, $name, $name2, $alias, $handle, $blockee, $blocker )
{
        global $rootserver;
        global $appname;
