...
<?php
    ...
    $result = do_mysqli_query("1","
            SELECT staff.providerid, providername, lastfunc, lastaccess, 
            timediff( now(), lastaccess ) as diff
            FROM braxproduction.staff

            left join provider on staff.providerid = provider.providerid
            where time_to_sec(timediff(now(),lastaccess))<60*60*24
